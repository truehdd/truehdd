...
### Added
- `verify` reports the maximum data rate and the access unit it peaked at, the maximum FIFO latency, access-unit size, and a per-substream table of depth against the depth allowed for that substream. The FIFO totals are broken into the stream's own bytes plus overhead, and labelled by the decoder each one belongs to, each summing only the substreams that decoder's presentation is made of. A decoder the stream does not carry reports no depth: a DVD-Audio stream whose only substream is six-channel has no two-channel decoder, and says so
...
- `info` reported the FBA `channel_meaning` fields for a DVD-Audio stream, which put an entirely different structure in those 64 bits, so dialogue and mix levels came out of bits that mean nothing of the sort. It now prints the FBB block — `fs`, `wordwidth`, `channel_occupancy`, `mlp_multi_channel_type`, `speaker_layout`, `copy_protection`, `level_control`, `hdcd_process`, `source_format` and `summary_info` — and lists only the presentations FBB declares, without the FBA per-presentation metadata that syntax does not carry
- `verify` reported a reserved-bits violation in `flags` and in `substream_info` on every major sync of a conformant DVD-Audio stream, plus an incompatibility between two fields only one of which exists in that syntax. Both rules were the FBA ones applied to both syntaxes
